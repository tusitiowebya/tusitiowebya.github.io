...
const fuObs = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    ...
    const parent = entry.target.closest(
      '.ben-grid, .prod-grid, .gal-grid, .faq-list, .porq-inner, .porq-cards, .cta-mid-inner, .cta-final-inner'
    );
    const delay = parent
...
function openLb(index) {
  lbIndex = index;
  lbImg.src = giItems[index].dataset.src || giItems[index].querySelector('img').src;
