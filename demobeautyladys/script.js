/* ============================================================
   BEAUTY LADY'S DISTRIBUIDORA — script.js (landing)
   ============================================================ */
...
const fuObs = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    ...
    const parent = entry.target.closest('.cat-grid, .ben-grid, .feat-grid, .proc-grid, .test-grid, .faq-list, .hero-content, .seg-grid');
...
if (featBox && typeof loadProducts === 'function') {
  loadProducts().then(list => {
    // un destacado por categoría, hasta 4
    const seen = new Set();
    const pick = [];
    for (const p of list) {
      if (!seen.has(p.cat)) { seen.add(p.cat); pick.push(p); }
      if (pick.length === 4) break;
    }
    featBox.innerHTML = pick.map(p => `
      <a class="feat-card fu" href="productos.html?cat=${encodeURIComponent(p.cat)}">
        <div class="feat-img"><img src="${p.img}" alt="${p.name}" loading="lazy" /><span class="feat-cat">${p.cat}</span></div>
        <div class="feat-body">
          <h3>${p.name}</h3>
