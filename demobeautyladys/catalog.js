/* ============================================================
   BEAUTY LADY'S DISTRIBUIDORA — catalog.js
   Render del catálogo, filtros, búsqueda y carrito → WhatsApp.
   ============================================================ */

const WA = "5491124521772";
...
let ALL = [];
let activeCat = "Todos";
let cart = JSON.parse(localStorage.getItem("beautyladys_cart") || "{}");
...
function setQty(id, qty){
  ...
  localStorage.setItem("beautyladys_cart", JSON.stringify(cart));
  render();
  updateCart();
}
...
function sendOrder(){
  ...
  const total = items.reduce((a, i) => a + i.qty * i.price, 0);
  let msg = "¡Hola Beauty Lady's! 💕 Quiero hacer este *pedido*:\n\n";
  items.forEach(i => { msg += `• ${i.name} — ${i.qty} ${i.unit}\n`; });
  msg += `\nTotal estimado: ${fmt(total)}\n(Aguardo confirmación de precio y stock 💄)`;
